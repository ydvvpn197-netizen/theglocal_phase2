...
class DraftManager {
  ...
  createDraft(
    ...
    logger.warn('📝 Created new draft', { draftId: draft?.id })
    return draft
  }
  ...
  updateDraft(draftId: string, updates: Partial<DraftData>): DraftData | null {
    ...
    if (draftIndex === -1) {
      logger.warn('Draft not found for update', { draftId })
      return null
    }
    ...
    logger.warn('📝 Updated draft', { draftId })
    return updatedDraft
  }
  ...
  async autoSaveDraft(draftId: string): Promise<boolean> {
    ...
    try {
      ...
      this?.syncDraftToServer(autoSavedDraft).catch((error) => {
        logger.warn('Server sync failed for draft', { draftId, error })
      })

      logger.warn('💾 Auto-saved draft', { draftId })
      return true
    } catch (error) {
      logger.error('Auto-save failed for draft', error as Error, { draftId })
      return false
    }
  }
  ...
  private async syncDraftToServer(draft: DraftData): Promise<void> {
    try {
      ...
      if (response?.ok) {
        ...
        logger.warn('☁️ Synced draft to server', { draftId: draft?.id })
      } else {
        logger.warn('Server sync failed', { statusText: response?.statusText })
      }
    } catch (error) {
      logger.error('Server sync error', error as Error)
      throw error
    }
  }
  ...
  private storeDrafts(drafts: DraftData[]): void {
    ...
    try {
      ...
      if (dataString?.length > DraftManager?.MAX_STORAGE_SIZE) {
        logger.warn('Draft storage size limit reached, removing oldest drafts')
      ...
      logger.error('Error storing drafts', error as Error)
      ...
      if (typeof window !== 'undefined') {
        ...
        try {
          ...
          logger.error('Failed to store reduced drafts', retryError as Error)
        }
      }
    }
  }
  ...
  deleteDraft(draftId: string): boolean {
    ...
    if (filteredDrafts?.length === drafts?.length) {
      logger.warn('Draft not found for deletion', { draftId })
      return false
    }
    ...
    this?.deleteDraftFromServer(draftId).catch((error) => {
      logger.warn('Server deletion failed', { error })
    })

    logger.warn('🗑️ Deleted draft', { draftId })
    return true
  }
  ...
  async recoverDrafts(userId: string): Promise<DraftData[]> {
    try {
      ...
      logger.warn('🔄 Recovered drafts from server', { count: serverDrafts?.length })
      return recoveredDrafts
    } catch (error) {
      logger.error('Draft recovery failed', error as Error)
      throw error
    }
  }
  ...
  private startAutoSave(): void {
    this.autoSaveTimer = setInterval(() => {
      if (this.pendingSaves.size > 0) {
        logger.warn('💾 Auto-saving pending drafts', { count: this.pendingSaves.size })
  ...
  private cleanupOldDrafts(): void {
    ...
    if (recentDrafts?.length < drafts?.length) {
      ...
      logger.warn('🧹 Cleaned up old drafts', { count: drafts?.length - recentDrafts?.length })
    }
  }
  ...
  clearAllDrafts(): void {
    ...
    this?.pendingSaves.clear()
    logger.warn('🧹 Cleared all drafts')
  }
  ...
  importDrafts(exportedData: string, userId: string): number {
    try {
      ...
      logger.warn('📥 Imported drafts', { count: userDrafts?.length })
      return userDrafts?.length
    } catch (error) {
      logger.error('Draft import failed', error as Error)
      throw error
    }
  }
}
