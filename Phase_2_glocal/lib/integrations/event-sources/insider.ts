/**
 * Insider?.in Integration
 * Real web scraping implementation using Playwright (for JavaScript-rendered content)
...
const INSIDER_BASE_URL = 'https://insider?.in'
...
async function getBrowserExecutablePath(): Promise<string | undefined> {
  ...
  if (process?.env.VERCEL || process?.env.AWS_LAMBDA_FUNCTION_NAME) {
    try {
      ...
      console?.warn('[Insider] @sparticuz/chromium not available, falling back to default')
      return undefined
    }
  }
  return undefined
...
/**
 * Fetch events from Insider?.in
 */
export async function fetchInsiderEvents(config: FetchConfig): Promise<PlatformFetchResult> {
  const startTime = Date?.now()

  try {
    // Check if Playwright is available
    if (typeof window !== 'undefined') {
      console?.warn('[Insider] Playwright not available in browser environment')
      return {
        platform: 'insider',
        success: true,
        events: [],
    ...
    const url = `${INSIDER_BASE_URL}/${citySlug}/events`

    // Check robots?.txt
    const robotsCheck = await globalRobotsChecker?.checkAccess(url)
    if (!robotsCheck?.allowed) {
      scraperLogger?.robotsViolation('insider', url)
      throw new Error('Scraping disallowed by robots?.txt')
    }

    scraperLogger?.log('insider', 'fetch_start', 'success', `Fetching events for ${config?.city}`)

    // Scrape events with rate limiting
    const events = await globalRateLimiter?.add('insider', () =>
      scrapeInsiderEvents(citySlug, config?.limit || 20)
    )
    ...
    scraperLogger?.failure('insider', 'fetch_failed', errorMsg, { city: config?.city }, duration)
...
/**
 * Scrape Insider?.in events using Playwright
 */
async function scrapeInsiderEvents(citySlug: string, limit: number): Promise<StandardizedEvent[]> {
  ...
  try {
    ...
    await page
      ?.waitForSelector(
        '[class*="event"], [class*="card"], [class*="listing"], a[href*="/event/"], [data-testid*="event"]',
        {
          timeout: 15000,
        }
      )
      ...
      for (const selector of selectors) {
        ...
        if (elements?.length > 0) {
          ...
          eventElements = elements?.filter((el) => {
            ...
            return link && link?.includes('/event/')
          })
    ...
    for (const scraped of scrapedEvents) {
      ...
      try {
        const event = processInsiderEvent(scraped, citySlug)
        if (event) {
          events?.push(event)
        }
      } catch (error) {
...
/**
 * Process scraped Insider?.in event data
 */
function processInsiderEvent(scraped: unknown, citySlug: string): StandardizedEvent | null {
  if (!scraped || typeof scraped !== 'object') return null
  const scrapedData = scraped as ScrapedInsiderEvent
  try {
    if (!scrapedData.title || !scrapedData.eventUrl) {
      return null
    }

    // Make URL absolute
    let eventUrl = scrapedData.eventUrl
    if (eventUrl?.startsWith('/')) {
      eventUrl = `${INSIDER_BASE_URL}${eventUrl}`
    }

    // Parse date - needed for ID generation
    const eventDate = parseDateText(scrapedData.dateText || '') || generateFutureDate()
    ...
    // Parse price
    let price = scrapedData.priceText || 'Check website'
    ...
    // Determine category
    const category = mapInsiderCategory(scrapedData.category || 'other')

    const event: StandardizedEvent = {
      external_id,
      title: scrapedData.title.trim(),
      description: `${scrapedData.title} - Experience this ${category} event at ${scrapedData.venue || cityName}. Book your tickets now!`,
      category,
      venue: scrapedData.venue || 'Venue TBD',
      city: cityName,
      ...
      genre: scrapedData.category || category,
      raw_data: {
        scraped_from: `${INSIDER_BASE_URL}/${citySlug}/events`,
        city_slug: citySlug,
        original_date_text: scrapedData.dateText,
      },
    }
...
/**
 * Parse date text from Insider?.in
 */
...
/**
 * Map Insider categories to standard categories
 */
function mapInsiderCategory(category: string): string {
