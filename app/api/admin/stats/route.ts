...
export const GET = withRateLimit(async function GET(_request: NextRequest) {
  try {
    ...
    const totalComments = commentsResult.count
    const activeArtists = activeArtistsResult.count
    const subscriptionsTrial = trialResult.count
    const subscriptionsActive = activeResult.count
