...
describe('Admin Dashboard Integration Tests', () => {
    ...
    it('should allow super admin to access admin routes', async () => {
      const createMockQueryBuilder = () => {
        const builder: {
          ...
          order: jest.fn().mockReturnThis(),
          limit: jest.fn().mockReturnThis(),
          then: jest.fn().mockResolvedValue({ count: 100, data: [] }),
        }
        return builder
      }
  ...
  describe('Platform Statistics', () => {
    it('should fetch comprehensive platform stats', async () => {
      ...
      mockFrom.mockImplementation((_table: string) => createMockQueryBuilder())
    ...
    it('should calculate active users correctly', async () => {
      const createMockQueryBuilder = (table: string) => {
        ...
        if (table === 'posts') {
          ...
          builder.then.mockResolvedValue({ count: 0, data: [] })
        }
        return builder
      }
      
      mockFrom.mockImplementation((table: string) => createMockQueryBuilder(table))
  ...
  describe('All Admin Endpoints - Comprehensive Tests', () => {
    it('should test all 14 admin GET endpoints with authorized access', async () => {
      ...
      mockFrom.mockImplementation(() => createMockQueryBuilder())
